...
export class UploadService {
  ...
  async processExcel(
    ...
    try {
      ...
      if (mapping) console.log(`[UploadService] Using custom mapping:`, mapping);
      ...
      if (maxScore <= 0) {
        throw new BadRequestException('Could not find a valid data sheet or header row in the Excel file.');
      }
      ...
      console.error(err);
      throw new BadRequestException(`Internal processing error: ${err.message}`);
    }
  }

  private getCol(row: any, aliases: string[], explicitHeader?: string) {
    if (explicitHeader && row[explicitHeader] !== undefined) return row[explicitHeader];
    const keys = Object.keys(row);
    const stdAliases = aliases.map(a => a.toLowerCase().replace(/[^a-z0-9]/g, ''));
    for (const key of keys) {
      if (stdAliases.includes(key.toLowerCase().replace(/[^a-z0-9]/g, ''))) return row[key];
    }
    return undefined;
  }

  private resolveValue(row: any, mappingValue: string | undefined, aliases: string[]) {
  ...
  async getHeaders(buffer: Buffer) {
    ...
    for (const name of workbook.SheetNames) {
      ...
      const rawData: any[] = xlsx.utils.sheet_to_json(sheet, { range: headerRowIndex, defval: null });
  ...
  private async syncPayslip(tenantId: string, riderId: string, month: number, year: number) {
    ...
    const netTotal = grossTotal + (payslip.bonus || 0) - (
      (payslip.deductions || 0) + (payslip.salesCash || 0) + (payslip.carRent || 0) + 
